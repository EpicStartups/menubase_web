...
const Store = {
  ...
  write(key, value) {
    try {
      ...
      window.dispatchEvent(new CustomEvent('menubase:store', {
        detail: {
          key
        }
      }));
  ...
  subscribe(key, fn) {
    const handler = e => {
      if (e.type === 'storage' && e.key === STORE_KEYS[key]) fn();else if (e.type === 'menubase:store' && e.detail?.key === key) fn();
    };
...
function evaluate(campaign, cart, ctx) {
  ...
  if (!withinSchedule(campaign, ctx)) return {
    eligible: false,
    value: 0,
  ...
  switch (campaign.type) {
    case 'threshold':
      return evalThreshold(campaign, cart);
    case 'happy_hour':
      return evalHappyHour(campaign, cart, ctx);
    case 'bundle':
      return evalBundle(campaign, cart);
    case 'weather':
      return evalWeather(campaign, cart, ctx);
    case 'inventory':
      return evalInventory(campaign, cart, ctx);
    case 'birthday':
      return evalBirthday(campaign, cart, ctx);
    case 'offpeak':
      return evalOffpeak(campaign, cart, ctx);
    case 'flash':
      return evalFlash(campaign, cart);
    case 'spin_wheel':
      return evalSpin(campaign, cart);
    case 'referral':
      return evalReferral(campaign, cart, ctx);
    case 'upsell_popup':
      return evalUpsell(campaign, cart);
    default:
      return {
        eligible: false,
        value: 0,
...
function evalThreshold(c, cart) {
  ...
  const value = estimateSavings('add_line', {
    skuId: rewardSku,
    equivalentSkuId: equivalent
  });
  return {
    eligible: true,
    op: 'add_line',
    payload: {
      skuId: rewardSku,
      equivalentSkuId: equivalent,
      displayPrice: sku?.price ?? 0
    },
    ui: {
      headline: c.config?.message || `FREE ${sku?.name || 'reward'}`,
      accent: 'lime'
    },
    value,
    reason: `Cart ≥ RM${min}`
...
function evalHappyHour(c, cart, ctx) {
  ...
  const value = estimateSavings('reveal_category', {
    categoryId
  });
  return {
    eligible: true,
    ...
    payload: {
      categoryId,
      visibleUntil: c.endTime || '17:00'
    },
    ui: {
      headline: 'Happy Hour active',
      subline: `Ends at ${c.endTime || '17:00'}`,
      accent: 'lime'
    },
    value,
...
function evalBundle(c, cart) {
  ...
  if (!a || !b) return {
    eligible: false,
    value: 0,
    reason: `needs ${cfg.groupA} + ${cfg.groupB}`
  };
  ...
  return {
    ...
    op: 'swap_sku',
    payload: {
      hideSkuIds: [a.id, b.id],
      // remove these two lines from cart on accept
      revealSkuId: comboSku,
      // add this one combo line
      mode: 'suggest' // doesn't auto-apply; customer must accept
    },
    ui: {
      headline: `Save RM${savings.toFixed(2)} with the combo`,
      subline: combo.name,
      accent: 'plum'
    },
    value: savings,
...
function evalWeather(c, cart, ctx) {
  ...
  if (ctx.weather !== trigger) return {
    eligible: false,
    value: 0,
    reason: `needs ${trigger}`
  };
  ...
  const labels = {
    rain: 'Rainy day menu unlocked ☔',
    hot: 'Hot day specials ☀️',
    cold: 'Cold day comforts ❄️'
  };
  return {
    eligible: true,
    op: 'reveal_category',
    payload: {
      categoryId,
      visibleUntil: 'while_weather_holds'
    },
    ui: {
      headline: labels[trigger] || 'Weather menu active',
      accent: 'plum'
    },
    value,
...
function evalInventory(c, cart, ctx) {
  ...
  if (stock >= cfg.threshold) return {
    eligible: false,
    value: 0,
    reason: `stock ok (${stock})`
  };
  ...
  if (!reveal) return {
    eligible: false,
    value: 0,
    reason: 'clearance SKU missing'
  };
  const value = estimateSavings('swap_sku', {
    hideSkuId: cfg.itemId,
    revealSkuId: clearanceSku
  });
  return {
    eligible: true,
    op: 'swap_sku',
    payload: {
      hideSkuIds: [cfg.itemId],
      // hide regular cheesecake
      revealSkuId: clearanceSku,
      // show last-chance cheesecake
      mode: 'auto' // applied automatically
    },
    ui: {
      headline: `Last ${stock} · ${reveal.name}`,
      subline: `RM${reveal.price.toFixed(2)}`,
      accent: 'plum'
    },
    value,
...
function evalBirthday(c, cart, ctx) {
  if (!ctx.customer.isBirthday) return {
    eligible: false,
    value: 0,
    reason: 'not your birthday'
  };
  if (cart.subtotal < (c.config?.minSpend || 25)) {
    return {
      eligible: false,
      value: 0,
      reason: `RM${c.config?.minSpend || 25} min spend`
    };
  }
  ...
  return {
    ...
    payload: {
      skuId: rewardSku,
      equivalentSkuId: sku?.twinOf || 'bc',
      displayPrice: 0
    },
    ui: {
      headline: '🎂 Happy birthday - free cheesecake',
      accent: 'lime'
    },
    value: estimateSavings('add_line', {
      skuId: rewardSku,
      equivalentSkuId: sku?.twinOf
    }),
...
function evalOffpeak(c, cart, ctx) {
  ...
  return {
    ...
    payload: {
      categoryId,
      visibleUntil: c.endTime
    },
    ui: {
      headline: 'Off-peak menu active',
      subline: `Ends at ${c.endTime || '16:00'}`,
      accent: 'plum'
    },
    value: estimateSavings('reveal_category', {
      categoryId
    }),
...
function evalFlash(c, cart) {
  ...
  if (cfg.endsAt && Date.now() > cfg.endsAt) return {
    eligible: false,
    value: 0,
    reason: 'flash ended'
  };
  ...
  return {
    ...
    payload: {
      categoryId,
      visibleUntil: cfg.endsAt
    },
    ui: {
      headline: 'Flash deal · ends soon',
      accent: 'lime'
    },
    value: estimateSavings('reveal_category', {
      categoryId
...
function evalReferral(c, cart, ctx) {
  if (!ctx.customer.referralCode) return {
    eligible: false,
    value: 0,
    reason: 'no code applied'
  };
  ...
  return {
    ...
    ui: {
      headline: 'Friend code applied · free coffee',
      accent: 'lime'
    },
    value: estimateSavings('add_line', {
      skuId: rewardSku,
  ...
  for (const p of prompts) {
    if (matchesUpsellCondition(p.condition, cart)) {
      ...
      return {
        ...
        op: 'suggest_line',
        payload: {
          skuId: p.suggestItemId,
          displayPrice: sku.price
        },
        ui: {
          headline: p.headline,
          subline: p.subline,
...
function resolve(campaigns, cart, ctx = DEFAULT_CONTEXT) {
  ...
  const blocked = evaluated.filter(e => !e.eligible).map(e => ({
    campaign: e.campaign,
    reason: e.reason,
    progress: e.progress
  }));
  ...
  const alternates = [...exclusive.slice(1), ...stackable.filter(e => e !== applied)];
...
function addToCart(cart, it, guestId = 'me') {
  ...
  let items;
  if (existing) {
    items = cart.items.map(i => i === existing ? {
      ...i,
      qty: i.qty + 1
    } : i);
  } else {
...
function removeFromCart(cart, itemId, guestId = 'me') {
  const items = cart.items.map(i => {
    if (i.id === itemId && i.guestId === guestId) return {
      ...i,
      qty: i.qty - 1
    };
    return i;
